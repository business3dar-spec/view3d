...
}).catch(err => { console.error('Startup failed:', err); process.exit(1); }); IF NOT EXISTS companies (
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='companies' AND column_name='bio') THEN
